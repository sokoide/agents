...
10. **elf2x68k 環境では IOCS ライブラリ関数が動作しない場合がある**。スプライト・グラフィック制御系は全て raw `trap #15` インラインアセンブラで呼び出す（`19_elf2x68kクロス開発.md` 参照）。
...
- elf2x68k 環境で IOCS ライブラリ関数を使っていないか（スプライト・グラフィック系は raw trap #15 必須）
