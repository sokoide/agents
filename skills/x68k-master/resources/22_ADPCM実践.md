...
ADPCM音声の録音・再生・効果音管理の実践的パターン。DMAコントローラCh3で転送。
...
**ブロック上限**: 65,280バイト ($FF00)。これを超えるとアレイチェーン/リンクアレイを使う。
...
再生開始後にIOCSが即座に返る。BGM用途に適する。

テーブル構造: `(addr.l, size.w, next.l)...(*, *, 0)`。nextが0で終端。
...
ヘッダなしの生ADPCMデータ。4ビットで1サンプルを符号化。
...
2. 差分の符号を符号ビットとして記録
3. 差分の絶対値を現在のステップ値で量子化 → 4ビットコード生成
4. コード上位4ビットがインデックス補正値の選択に使われる
5. ステップインデックスを補正 (0-48の範囲にクランプ)
6. 予測値を更新
7. **ビットパッキング**: 下位4ビットが先 (リトルエンディアン的配置)
...
複数PCMファイルを1つのライブラリファイルに結合するフォーマット。
